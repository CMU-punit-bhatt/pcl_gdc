...
GRID_SIZE = 0.1
index_field_sample = np.full((35, int(80 / 0.1), int(80 / 0.1)), -1, dtype=np.int32)
...
def subsample_mask_by_grid(pc_rect):
    ...
    pc_rect_quantized[:, 0] = pc_rect_quantized[:, 0] + int(80 / GRID_SIZE / 2)
    pc_rect_quantized[:, 1] = pc_rect_quantized[:, 1] + int(1 / GRID_SIZE)

    index_field = index_field_sample.copy()

    index_field[
        pc_rect_quantized[:, 1], pc_rect_quantized[:, 2], pc_rect_quantized[:, 0]
    ] = np.arange(pc_rect.shape[0])
...
def GDC(
    ...
    consider_PL = (
        filter_mask(ptc)
        * filter_theta_mask(
            ptc, low=np.radians(consider_range[0]), high=np.radians(consider_range[1])
        )
    ...
    consider_L = filter_mask(depth2ptc(gt_depth, calib)).reshape(gt_depth.shape)
    ...
    ptc = np.concatenate((ptc[pred_mask.reshape(-1)], ptc[gt_mask.reshape(-1)]))
    if verbose:
    ...
    W_PLL = csr_matrix(
        (W[N_PL:][idx_PLL], neighbors[N_PL:][idx_PLL], indptr_PLL), shape=(N_L, N_PL)
    )

    idx_LL = neighbors[N_PL:] >= N_PL
    indptr_LL = np.concatenate(([0], np.cumsum(idx_LL.sum(axis=1))))
    W_LL = csr_matrix(
        (W[N_PL:][idx_LL], neighbors[N_PL:][idx_LL] - N_PL, indptr_LL), shape=(N_L, N_L)
    )
    ...
    ATA = LinearOperator((A.shape[1], A.shape[1]), matvec=lambda x: A.T.dot(A.dot(x)))
