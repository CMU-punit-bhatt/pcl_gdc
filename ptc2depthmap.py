...
import numpy as np
from utils.kitti_util import Calibration, load_velo_scan, load_image
from utils.kitti_object import get_lidar_in_image_fov
from tqdm.auto import tqdm
from multiprocessing import Process, Queue, Pool


def get_ptc_in_image(ptc, calib: Calibration, img):
...
def get_depth_map(ptc, calib:Calibration, img):
...
def convert_ptc2depthmap(ptc: np.ndarray, calib: Calibration, img: np.ndarray) -> np.ndarray:
    """
    Convert pointcloud to depth map
...
############## below code works only for kitt_object dataset. Not for kitti raw ###############
